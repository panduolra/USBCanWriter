...
void usb_sendProcess()
{
  ...
  unsigned char i;
  for (i = 0; i < length; i++) {
    if ((usb_sendbuffer == usb_dev_desc + USB_DEV_DESC_SERIALNUMBER_OFFSET) && usb_serialNumberAvailable())
      ep0in_buffer[i] = USB_STRING_SERIALNUMBER_INDEX;
    else
...
unsigned char usb_handleDescriptorRequest(unsigned char type, unsigned char index, unsigned short length)
{
  ...
  case DESCR_STRING:
    switch (index) {
    case 0:
      return usb_loadDescriptor(usb_string_0, sizeof(usb_string_0), length);
    case 1:
      return usb_loadDescriptor(usb_string_manuf, sizeof(usb_string_manuf), length);
    case 2:
      return usb_loadDescriptor(usb_string_product, sizeof(usb_string_product), length);
    case 3:
      return usb_loadDescriptor(usb_string_serial, USB_STRING_SERIALNUMBER_SIZE, length);
    }
  }
